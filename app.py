...
- Watchlist (session-state based)
- Portfolio tracker (max 5 holdings)
- Top Picks (Day Trade + Swing Trade with hold estimates)

NOTES:
- Data from PSX Data Portal for personal, non-commercial use
- yfinance as fallback when PSX unavailable
- All signals are technical estimates, not guaranteed

DEPLOYMENT:
- Python 3.11+ (tested on 3.12)
- requirements.txt: pandas>=2.0.0, numpy>=1.24.0
"""
...
# ============================================================
# CSS - DARK THEME (Phase 3 mein light hoga)
# ============================================================
...
# ============================================================
# PSX OFFICIAL DATA PORTAL - Phase 1 (verified)
# ============================================================
...
def fetch_psx_official_index_json(index_symbol: str = "KSE100") -> Tuple[Optional[pd.DataFrame], str, str]:
    """
    PSX EOD JSON fetch. Verified format:
    {"data": [[timestamp, close, volume, open], ...]}
...
def rsi(series, period=14):
    """Wilder RSI - corrected. Handles all edge cases."""
    delta = series.diff()
...
def risk_engine(d, sr, breakout_status=""):
    """Risk calc with validation. Returns error field if invalid."""
    last = d.iloc[-1]
    price = last["Close"]
    atr_val = last["ATR14"] if not pd.isna(last["ATR14"]) else 0
...
def signal_engine(d, trend, trend_score, momentum, breakout, pullback, sr, risk_data, market):
    ...
    if trend_ok:
        if components["trend"] >= 70: reasons.append(f"✅ Trend: {trend}")
        elif components["trend"] <= 35: reasons.append(f"❌ Trend: {trend}")
        if components["momentum"] >= 70: reasons.append(f"✅ Momentum: {momentum['label']}")
        elif components["momentum"] <= 35: reasons.append(f"❌ Momentum: {momentum['label']}")
        if "CONFIRMED" in breakout["status"]: reasons.append(f"✅ {breakout['status']}")
        elif "BREAKOUT READY" in breakout["status"]: reasons.append(f"📌 {breakout['status']}")
        if pullback["status"] == "HEALTHY PULLBACK": reasons.append("✅ Healthy pullback")
        elif pullback["status"] == "BROKEN SUPPORT": reasons.append("❌ Support broken")
        if momentum["overbought"]: reasons.append("⚠️ RSI overbought")
        if momentum["oversold"]: reasons.append("📌 RSI oversold — reversal watch")
        if not rr_ok: reasons.append(f"❌ R:R kam hai ({round(risk_data['rr1'],2) if risk_data['rr1'] else 'N/A'})")
        if market["regime"] == "BULLISH": reasons.append("✅ Market bullish")
        elif market["regime"] == "BEARISH": reasons.append("❌ Market bearish")
...
def estimate_hold_period(result: Dict, category: str = "swing") -> str:
    ...
    Day trade: 1-2 sessions
    Swing: distance_to_T1 / (ATR14 * 0.5), capped 3-20 sessions
    """
    if category == "day":
    ...
    try:
        ...
        # ATR * 0.5 = realistic daily progress (choppy market adjustment)
...
def portfolio_decision(holding: Dict, result: Dict) -> Tuple[str, str]:
    """Original stop + technical stop + active stop (max of both). None-safe."""
    if result is None:
...
def build_chart(result, show_bb=False, show_sma200=False,
    ...
    fig.add_annotation(
        ...
        bgcolor="rgba(14, 17, 23, 0.8)", bordercolor=trend_color,
...
def get_indicator_explanation(indicator: str) -> str:
    explanations = {
        "SMA20": "20 din ki average price. Price iske upar ho to short-term strength.",
        "SMA50": "50 din ki average. Medium-term trend batata hai.",
        "RSI": "Momentum 0-100. 70+ overbought, 30- oversold.",
        "MACD": "Momentum direction. Positive histogram = bullish.",
        "ADX": "Trend strength. 25+ = strong trend.",
        "RVOL": "Aaj ka volume normal se kitna zyada.",
        "Support": "Jahan buyers aate hain — price girke wapas uthta hai.",
        "Resistance": "Jahan sellers aate hain — price chadhke wapas girta hai.",
        "Breakout": "Price resistance ke upar close ho with volume.",
        "Stop Loss": "Yahan exit karo loss limit karne ke liye.",
        "Target": "Technical price objective.",
        "ATR": "Average daily movement — stop-loss set karne mein help.",
    }
...
with tab_screener:
    ...
    include_penny = st.checkbox(
        "Include penny stocks (below PKR {})".format(int(penny_threshold)),
        value=False,
